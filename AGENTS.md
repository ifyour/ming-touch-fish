...
- **表结构自动迁移**：better-auth 首次请求时自动建表（`user` / `session` / `account` / `verification` 以及本项目的自定义 `read_articles`）。`read_articles` 表以 better-auth **插件 schema** 方式定义（`apps/web/app/server/auth.ts` 的 `readArticlesPlugin`），与 better-auth 自身表共用一套迁移，无需单独 drizzle 迁移。**改了 auth 表结构后**需对远程 D1 触发一次 `POST /api/admin-migrate`（本地 dev 也会在首次请求时自动建）。
...
- `GET /api/articles/:id/summary`：文章 **AI 总结**。先查 D1 `summary` 字段，命中缓存直接返回 `{ summary, cached: true, stage: 'cached' }`；未命中则按**四级回退**抽取正文，任一级抽到「充足正文」（清洗后 ≥ 80 字且有效字符占比 ≥ 30%，见 `isContentSufficient`）即用其调 Gemini，全部不足才 422。**阶段顺序经过优化**：先读库（最便宜）→ 再 live 抓页（最慢最易被反爬）→ 最后才 browser（最贵），让绝大多数请求在零出站的第一步命中，避免每次总结都发起 10~20s 出站抓取。成功响应返回 `{ summary, cached: false, stage: <命中阶段>, stageMs: {各阶段耗时ms} }`，`stage` 与失败路径口径一致，便于成功/失败统一排查瓶颈：
  1. **读库预抽正文（rss-content）**：优先用 fetcher 入库时**预抽的纯文本** `metadata.fullContent`（fetcher 在 `fetchAndStore` 落库阶段即对 RSS `content`/`content_rendered`/`description` 跑一遍 `extractFromHtml()`，把清洗后的纯文本正文存进 `metadata.fullContent`，截断前 8000 字符；满足 `≥ 80 字` 才落库，否则留空串）。该字段直接读库、零出站，对「RSS 缺正文 + 实时抓被反爬」的源可跳过 live/browser 两级直接命中——**这是性价比最高的来源，绝大多数请求在此命中**。若 `fullContent` 为空，再回退到 `metadata.content`（fetcher 落库的原始 HTML 片段 `content:encoded` / `content` / `content_rendered`），走 `extractFromHtml()` 现抽（此分支为保底，正常落库应有 fullContent）。
  2. **实时抓取文章 URL（live）**：仅当前一步未命中才跑。`extractArticleText()` → `fetchWithUA()`（`@repo/shared` 多 UA 回退）+ `linkedom` 构造 DOM + `@mozilla/readability` 提取正文。`extractArticleText()` **先用 `PAYWALL_UAS`（`Googlebot/2.1` + `bingbot/2.0`，真爬虫 UA）抓页**——多数媒体站对搜索引擎放开全文本以便收录，故爬虫 UA 常能绕过 JS 注入的付费墙遮罩；失败再用普通浏览器 UA（`SUMMARY_UAS`）兜底。`fetchWithUA()` 接受覆盖 UA 列表参数（`fetchWithUA(url, init, uas)`），`extractArticleText` 依次试 `[PAYWALL_UAS, undefined（默认 SUMMARY_UAS）]` 两档。`extractFromHtml()` 内部**优先解析 `application/ld+json` 结构化数据里的 `articleBody`**（覆盖根级与 `@graph` 嵌套）——多数媒体站为 SEO 把全文塞进 JSON-LD，而前端付费墙 JS 只遮挡可见层，故 JSON-LD 能拿到完整正文、绕过弹窗。取不到再走 Readability。该路径**关闭去标签兜底**（`extractFromHtml({ allowStripFallback: false })`）——Readability 取不到正文就抛错，避免把 JS 渲染页 shell 当正文、绕过下方 RSS 回退并缓存垃圾。**约束（已踩坑）**：`extractFromHtml()` 喂给 Readability 的必须是**仅剔除 `<script>`/`<style>` 但保留其余标签的 HTML**，绝不能先把整段 HTML 去标签成纯文本再喂。一旦去标签，linkedom 解析出的 `document.documentElement` 为 `null`，Readability 直接抛错被 `try/catch` 吞掉，导致**所有 live 实时抓取的正文抽取静默失效**、一路退到 Browser 阶段（煎蛋网等 RSS 仅含短 description 的源因此必走到浏览器渲染）。去标签纯文本仅作最后保底分支。**live 反爬/拦截页识别**：抓到的挑战页（"请启用 JavaScript" / "verify you are human" 等）文本可能 ≥80 字但无意义，当成正文会喂给 Gemini 生成幻觉总结并污染缓存，故 `extractArticleText()` 结果会过 `looksLikeErrorPage()`（与 browser 阶段共用同一组拦截特征词）判定，命中即视为无效、回退下一级。**live 不写失败缓存**：live 失败多为稳定反爬，不写 `liveFailedAt` 类标记——1h 短缓存收益低，而每次失败都重写整个 `metadata` JSON 列反而消耗更稀缺的 D1 写配额（Free 50k 写/天）。失败直接回退后续阶段，靠 browser 兜底即可。
  2.5 **付费墙友好 AMP 子步骤**：live 抓到挑战页或正文不足时（`stage` 仍为 `live`/`rss-content`），再调 `extractArticleTextViaAmp(url)` 试 AMP 变体——对 URL 自动拼 `/amp`、`?amp=1`、`?outputType=amp` 三种变体（已带 `amp` 字样的 URL 不重复拼接），逐变体 `fetchWithUA()` 抓页后走 `extractFromHtml()`；命中 `looksLikeErrorPage()` 视为无效，命中 `isContentSufficient()` 即采用（`source='live'`）。多数媒体站对 AMP 版本放开付费墙，是个几乎零成本的补充来源，失败再继续走第 3 级 RSS 简介回退。变体生成逻辑在 `apps/web/app/server/lib/extract.ts` 的 `buildAmpVariants()`（内部函数，仅处理合法 URL，非法输入返回空数组、整步跳过）。
  3. **回退 RSS 简介（rss-desc）**：`metadata.description`（采集的 `summary ?? description`），走 `extractFromHtml()`。与第 1 级同属「读库现抽」，逻辑合并、仅保留阶段标签；对只有短 description 的源（如煎蛋网 RSS 0.92）通常不足 80 字而失败。
  4. **第四层 Browser Rendering 真浏览器渲染**：前三层全失败 + 配置了 `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` + 未在失败缓存期（见下）时触发，调 `extractArticleTextViaBrowser()` → 走 **Browser Run REST API**（`POST https://api.cloudflare.com/client/v4/accounts/{accountId}/browser-rendering/markdown`，带 `Authorization: Bearer <token>`），取 markdown 正文截断前 8000 字符。治「实时抓取被反爬拦截、但内容其实是免费完整正文」的源（V2EX 等 JS 渲染 / 轻度反爬站）。`gotoOptions` 用 `waitUntil: 'load'` + 超时 15s（非 networkidle2/30s），正文 markdown 提取不需要等所有异步资源，避免吃 Browser 免费时长（每天 10 分钟）。失败仅 `logger.warn` 后继续走 422，不影响其他回退。日志 `logger.info('文章总结使用 Browser Rendering 生成', { source: 'browser', browserMsUsed })` 带浏览器耗时（读响应头 `X-Browser-Ms-Used`）。**错误页拦截**：浏览器渲染拿到的 markdown 可能是「页面崩溃 / JS 报错」式错误页（如 Kaggle 讨论页 SPA 崩溃，正文只剩 "Something went wrong and this page crashed!" + SyntaxError 堆栈）。`extractArticleTextViaBrowser()` 命中崩溃/拦截特征（`something went wrong and this page crashed` / `is not valid json` / `unexpected token` / `enable javascript and cookies` / `checking your browser` / `verify you are human` / `access denied` 等，大小写不敏感）即视为无效、抛错交路由回退到 422，避免把噪声当正文喂给 Gemini 生成幻觉总结并污染 `summary` 缓存（真实踩坑：article 7261 曾因此返回「该页面因服务器错误导致崩溃…JSON解析错误」式幻觉总结）。长度不足由上层 `isContentSufficient` 判定，错误页检测只依赖关键词。
  ...
  - **回退可观测性**：实时抓取失败会打 `logger.warn('文章总结实时抓取失败，回退 RSS 简介')`（带 `ms` 耗时）；最终用回退来源生成时会打 `logger.info('文章总结使用回退来源生成', { source: 'rss-content' | 'rss-desc' | 'browser' | 'live' })`。成功响应带 `stageMs`（各阶段耗时 ms），失败响应带 `stage`，所有日志带 `service: 'web-api'` 与 `articleId`，便于在 Cloudflare 控制台筛选回退占比与定位瓶颈。
  - **正文抽取不足（四级回退后仍 < 80 字或有效字符占比 < 30%，多见于 V2EX 等 JS 渲染页 / 站点首页且 RSS 也无正文）直接返回 422 `{ error: '正文抽取不足', stage, detail }`，不调 Gemini、不写库，可后续重试**；其他抽取 / 总结错误返回 500 `{ error, stage }`，不回退标题。**失败缓存**：四级全失败时写 `metadata.summaryFailedAt: Date.now()`（合并保留原有 `content`/`description`，TTL 24h），24h 内同一文章不再触发第四层浏览器渲染，自适应处理「已知不可抓」的源（如经济学人 bit.ly 短链 + 付费墙，第一次白跑一次超时不计费的浏览器，之后被缓存拦下）。`recentlyFailed(metadata)` 读 `summaryFailedAt` 判断是否 24h 内。其他抽取 / 总结错误返回 500 `{ error, stage }`，不回退标题。注意：V2EX 等源因 fetcher 已落库 `content_rendered`，通常能在第 1 级命中，不再 422。`stage` 为诊断字段，标识成功/失败所处阶段：`cached`（命中缓存，仅成功响应）、`rss-content`（读库预抽正文）、`live`（实时抓取+Readability）、`rss-desc`（回退 RSS 简介）、`browser`（Browser Rendering 渲染抓取）、`gemini`（调 Gemini 总结），聚合该字段可定位整条抓取流程中具体薄弱的站点/阶段。422 的 `detail` 进一步区分「各阶段均未能抽取到任何正文」与「抽取到的正文不足（字数或有效字符占比不达标）」。
  - **Browser Rendering 绑定与限制**：**Pages Functions 不支持 Browser 绑定**（会导致整个 worker 初始化崩溃、全站 500，已踩坑验证），故第四层改用 **Browser Run REST API**（`extractArticleTextViaBrowser()` 内 `fetch` 调用），不依赖任何绑定。所需环境变量：`CLOUDFLARE_API_TOKEN`（带 `Browser Rendering - Edit` 权限的 API Token，作为 secret 配到 Pages 项目）、`CLOUDFLARE_ACCOUNT_ID`（账户 ID）。两者任一缺失则静默跳过第四层，行为同改造前。**Browser Run 在 Free 计划可用**：每天 10 分钟浏览器时长免费，超出仅返回 429 限流、不扣费；失败请求不计费。计费：Free 每天 10 分钟 / Paid 每月 10 小时（超出 $0.09/小时）。本地 dev 通常不配置这俩变量，第四层自动跳过。不做域名黑名单、不加 KV 层、不动 fetcher / Firecrawl、不改前端（第四层对前端透明）。
...
- `GET /api/articles/grouped`：按源聚合文章。**支持 `scope` 查询参数**——`active`（默认）只返回「最新文章 30 天内」的活跃源，`stale` 只返回过期源。服务端先用 `GROUP BY` + `MAX(publishedAt)`（走 `articles_source_published_idx` 索引）算出每源最新发布时间做过期判定，再仅对目标源集合逐源 `ORDER BY published_at DESC LIMIT 10` 取文章（每源一次索引查询、1 个绑定参数，不走窗口函数全量扫描、也不拼超长 IN 列表触发 D1 参数上限），从源头压低 D1 读配额。活跃查询额外返回响应头 `X-Has-Stale: true/false` 指示是否存在可懒加载的过期源，并带 `Cache-Control: public, max-age=300, stale-while-revalidate=600` 边缘缓存——首页数据最快每小时（fetcher cron）才变，长缓存把首页查库频率砍约 15 倍；SWR 保证过期后用户仍瞬时命中旧内容、后台异步刷新。返回结构每个元素带 `total`（该源文章总数）。**单源分页**：带上 `sourceId` 时进入单源模式，按 `offset`（默认 0）、`limit`（默认 10，上限 50）取该源下一批文章，返回单个元素的数组，响应头 `X-Has-More: true/false` 指示是否还有更多；前端首页「Show more」/滚动到底部时调用此模式实现无限滚动。**后台操作后即时可见**：后台增删改源/手动抓取成功后，前端 `fish.tsx` 调 `bumpGroupedBust()`（见 `apps/web/app/utils/groupedBust.ts`）后 `invalidateQueries(['articles','grouped'])`；首页 `fetchGroupedArticles` 携带 `?bust=<token>` 使 URL 不同、穿透边缘长缓存立即拿到最新数据。普通读者不 bump，继续命中长缓存，配额收益不受影响。token 仅存前端内存，刷新页面即重置。
...
- **正文抽取 `extractFromHtml()` 的 Readability 必须吃真实 DOM，不能先去标签**：`@mozilla/readability` 需要带标签结构的 HTML。若先 `replace(/<[^>]+>/g,' ')` 变成纯文本再 `parseHTML()`，linkedom 解析出 `documentElement` 为 `null`，Readability 抛 `Cannot destructure 'firstElementChild'`，异常被吞后 live 抓取第 1 级恒失败，依赖 live 的源会一路退到第 4 级 Browser Rendering（煎蛋网 RSS 0.92 只有短 description、无 content 字段，必中此坑）。正确做法见 `packages/shared/src/extract.ts:68`：只剔除 `<script>`/`<style>` 块、保留其余标签交给 Readability，去标签纯文本仅作保底。改动 `extract.ts` 后务必用带正文的文章页（如煎蛋 `https://jandan.net/p/xxxx`）实测第 1 级能抽出 ≥80 字正文。
