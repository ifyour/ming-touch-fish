...
- `GET /api/articles/:id/summary`：文章 **AI 总结**。先查 D1 `summary` 字段，命中缓存直接返回 `{ summary, cached: true }`；未命中则按**四级回退**抽取正文，任一级抽到「充足正文」（清洗后 ≥ 80 字且有效字符占比 ≥ 30%，见 `isContentSufficient`）即用其调 Gemini，全部不足才 422：
  1. **实时抓取文章 URL**：`extractArticleText()` → `fetchWithUA()`（`@repo/shared` 多 UA 回退）+ `linkedom` 构造 DOM + `@mozilla/readability` 提取正文。`extractFromHtml()` 内部**优先解析 `application/ld+json` 结构化数据里的 `articleBody`**（覆盖根级与 `@graph` 嵌套）——多数媒体站为 SEO 把全文塞进 JSON-LD，而前端付费墙 JS 只遮挡可见层，故 JSON-LD 能拿到完整正文、绕过弹窗。取不到再走 Readability。该路径**关闭去标签兜底**（`extractFromHtml({ allowStripFallback: false })`）——Readability 取不到正文就抛错，避免把 JS 渲染页 shell 当正文、绕过下方 RSS 回退并缓存垃圾。**约束（已踩坑）**：`extractFromHtml()` 喂给 Readability 的必须是**仅剔除 `<script>`/`<style>` 但保留其余标签的 HTML**，绝不能先把整段 HTML 去标签成纯文本再喂。一旦去标签，linkedom 解析出的 `document.documentElement` 为 `null`，Readability 直接抛错被 `try/catch` 吞掉，导致**所有 live 实时抓取的正文抽取静默失效**、一路退到 Browser 阶段（煎蛋网等 RSS 仅含短 description 的源因此必走到浏览器渲染）。去标签纯文本仅作最后保底分支。
  2. **回退 RSS 正文**：优先用 fetcher 入库时**预抽的纯文本** `metadata.fullContent`（fetcher 在 `fetchAndStore` 落库阶段即对 RSS `content`/`content_rendered`/`description` 跑一遍 `extractFromHtml()`，把清洗后的纯文本正文存进 `metadata.fullContent`，截断前 8000 字符；满足 `≥ 80 字` 才落库，否则留空串）。该字段直接读库、零出站，对「RSS 缺正文 + 实时抓被反爬」的源可跳过 live/browser 两级直接命中。若 `fullContent` 为空，再回退到 `metadata.content`（fetcher 落库的原始 HTML 片段 `content:encoded` / `content` / `content_rendered`），走 `extractFromHtml()` 现抽去标签兜底。
  3. **回退 RSS 简介**：`metadata.description`（采集的 `summary ?? description`），同样走 `extractFromHtml()`。
  4. **第四层 Browser Rendering 真浏览器渲染**：前三级全失败 + 配置了 `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` + 未在失败缓存期（见下）时触发，调 `extractArticleTextViaBrowser()` → 走 **Browser Run REST API**（`POST https://api.cloudflare.com/client/v4/accounts/{accountId}/browser-rendering/markdown`，带 `Authorization: Bearer <token>`），取 markdown 正文截断前 8000 字符。治「实时抓取被反爬拦截、但内容其实是免费完整正文」的源（V2EX 等 JS 渲染 / 轻度反爬站）。失败仅 `logger.warn` 后继续走 422，不影响其他回退。日志 `logger.info('文章总结使用 Browser Rendering 生成', { source: 'browser', browserMsUsed })` 带浏览器耗时（读响应头 `X-Browser-Ms-Used`）。**错误页拦截**：浏览器渲染拿到的 markdown 可能是「页面崩溃 / JS 报错」式错误页（如 Kaggle 讨论页 SPA 崩溃，正文只剩 "Something went wrong and this page crashed!" + SyntaxError 堆栈）。`extractArticleTextViaBrowser()` 命中崩溃/拦截特征（`something went wrong and this page crashed` / `is not valid json` / `unexpected token` / `enable javascript and cookies` / `checking your browser` / `verify you are human` / `access denied` 等，大小写不敏感）即视为无效、抛错交路由回退到 422，避免把噪声当正文喂给 Gemini 生成幻觉总结并污染 `summary` 缓存（真实踩坑：article 7261 曾因此返回「该页面因服务器错误导致崩溃…JSON解析错误」式幻觉总结）。长度不足由上层 `isContentSufficient` 判定，错误页检测只依赖关键词。
