...
**注意：管理后台路径是 `/fish`，不是 `/admin`。** README 中提到的 `/admin` 已过时。前端通过点击页头 "TouchFish News" 文字 **3 次**（1.5 秒内）跳转到 `/fish`，规避直接暴露。后台无鉴权，仅靠隐蔽入口；如需保护请用 Cloudflare Access。
...
**硬性约束**：当进行任何重大功能变更或大规模系统调整时，Agent 必须同步更新本文件（`AGENTS.md`）与项目根目录的 `README.md`，不得事后补登。

### 触发范围

以下任一情况均视为"重大变更"，必须同步文档：
...
- 可能影响用户交互或运维的关键信息（如新的鉴权入口、新的环境变量、 breaking change）

### 提交要求

- 文档更新与代码变更**放在同一次提交**中，不得拆分到后续提交。
- 若改动涉及多个文档（AGENTS.md + README.md），需同时更新两处；只改一处视为未完成。
- 提交信息按 [提交规范](#提交规范) 正常书写，文档同步不单独成提交。

目标：始终维持项目文档对系统整体能力的准确、全面且最新的描述。
