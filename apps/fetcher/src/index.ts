  ...
  async scheduled(_controller: ScheduledController, env: Env, _ctx: ExecutionContext): Promise<void> {
    ...
    for (const source of sources) {
      try {
        ...
        logger.info(`Queued source ${source.id}: ${source.name}`, { service: 'fetcher', sourceId: source.id });
      } catch (err) {
        logger.error(`Failed to queue source ${source.id}`, { service: 'fetcher', sourceId: source.id, error: err });
      }
    }
  },
  ...
  async queue(batch: MessageBatch<QueueMessage>, env: Env, _ctx: ExecutionContext): Promise<void> {
    for (const message of batch.messages) {
      ...
      try {
        ...
        if (quotaMsg) {
          logger.error(`Quota error for source ${sourceId}, acking (retry won't help): ${quotaMsg}`, { service: 'fetcher', sourceId, error: err });
          message.ack();
        } else {
          logger.error(`Failed to process source ${sourceId}, will retry`, { service: 'fetcher', sourceId, error: err });
          message.retry();
        }
      }
    }
